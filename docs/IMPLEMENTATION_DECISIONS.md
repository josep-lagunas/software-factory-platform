...
- Agent operations boundary: the Workspace Worker/coder performs repository operations directly via the Git Provider Adapter (no raw provider credentials; inside the sandbox, ID-060), while workflow-control side effects (Jira status transitions, merge execution, user notifications) remain centralized in the Orchestrator. This is already the MAS model (§9.6); recorded to remove ambiguity.
