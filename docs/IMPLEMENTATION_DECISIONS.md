...
- **Foreign-key policy (normative):**
  ...
  - **In-code deferral marker** (SFP-235): a declared deferral is marked *on the ORM column itself* via SQLAlchemy's tooling dict — `mapped_column(..., info={"deferred_fk": "<schema.table.column>", "blocked_on": "<SFP-XXX>"})` (or `Column(..., info={...})`). `deferred_fk` is the FK obligation to be closed when the target lands; `blocked_on` is the ticket delivering the target aggregate. The marker is purely declarative — `info` does not change runtime behavior — and is the in-code artifact that the `tools/fk_lint.py` enforcement (SFP-235) recognizes, so a deferral is machine-visible at the model, not just in the PRSpec. The PRSpec field and the in-code marker are complementary: the PRSpec entry is the *planning-time* declaration, the `info` marker is the *code-time* declaration that survives into the committed model.
  - **Enforcement** (SFP-235): `tools/fk_lint.py` introspects each service's `Base.metadata.tables` — the FK source of truth, NOT the Alembic migrations — and fails on any plain `_id` column whose inferred target table IS present in the same service's metadata (a same-service reference that lost its FK). Real FKs (any SQLAlchemy style), declared deferrals (the `info` marker, target intentionally absent), and cross-service identifiers (target not in this metadata) all pass. The lint is wired into CI per service alongside mypy/ruff.
...
### Consequences
Positive: uniform, ownership-respecting, clean separation of business and operational state; real referential integrity within each service database; cross-ticket sequencing deferrals are declared and tracked rather than silently dropped; the SFP-235 in-code `info` marker + `tools/fk_lint.py` make the policy machine-enforced, so a deferral is visible at the committed model and a missing same-service FK cannot recur silently.
Negative: per-service Alembic discipline and schema conventions must be maintained; the bidirectional deferral protocol requires discipline — originating tickets must record a `deferred_fk_obligations` PRSpec entry AND mark the column with the `info={"deferred_fk", "blocked_on"}` marker, and target tickets must close the deferral on landing (SFP-234 backfilled the PRSpec entry for `Ticket.project_id → business.projects`, blocked on SFP-100; SFP-235 backfilled the in-code marker).
...
- Two layers:
  1. A deterministic readiness rubric, rule-checked where possible, verifying the ticket carries the mandatory sections of the AI Implementation Specification template (ID-070): Context, Requirements, Files to create/modify, Implementation notes, References, Context outputs / required inputs, and Acceptance criteria (plus Dependencies in metadata). A missing required section is an automatic Not-Ready with a specific reason, and no model call. The rubric's input is a typed `ParsedTicket` (a pydantic model of the ID-070 sections); the markdown→ParsedTicket parser is a separate concern. The rubric reuses `ReadinessOutput` — it populates `rubric_results` (section→bool), pushes a per-section reason into `blocking_ambiguities`, and sets the verdict.
