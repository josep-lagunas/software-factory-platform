...
def _no_network(monkeypatch):
    ...
    def _boom(*_args, **_kwargs):
        raise AssertionError("network call detected — mock cjt.jira_api / cjt.create_issue instead")
...
def test_TC004_area_in_summary_and_labels(tmp_path, monkeypatch):
    ...
    assert fields["summary"] == f"[SPECIAL] {BOT_EMOJI} T"  # byte-identical
...
def test_TC005_create_payload_deep_equal(tmp_path, monkeypatch, capsys):
    ...
    create_sfp_ticket.main(
        [
            ...
            "x,y",
            "--description",
            _write_body(tmp_path, body_text),
        ]
    )
    expected = {
        "project": {"key": "SFP"},
        "issuetype": {"name": "Task"},
        "summary": f"[SPEC] {BOT_EMOJI} Title",
...
def test_TC006_update_existing_str(tmp_path, monkeypatch, capsys):
    ...
    api = _patch_jira_api(
        monkeypatch, {"fields": {"description": "**Ctx:** markdown str", "labels": ["keep"]}}
    )
    create_sfp_ticket.main(
        [
            "--update",
    ...
    assert put_fields["summary"] == f"[SPEC] {BOT_EMOJI} T"
